...
static const auto
    ...
    score_color{Color{0x44, 0x44, 0x4e, 0xff}},
...
static struct {
    ...
    std::uniform_int_distribution<int> _y_dist{-10, 10};

    void set_random_velocity(void) noexcept {
        // y axis velocity
        int yv = _y_dist(_rng);
        ...
        velocity = Vector2{
            (time(nullptr) & 1) ? -ball_x_velocity : ball_x_velocity,
            SC(float, yv)
        };
    }
